...
int gelic_init(void)
{
	...
	if (result != 0)
		return result;*/
	result = map_dma_mem(GELIC_BUS_ID, GELIC_DEV_ID, dma, sizeof(dma), &dma_bus_addr);
	if (result != 0)
		return result;
...
int gelic_deinit(void)
{
	...
	//lv1_net_set_interrupt_mask(GELIC_BUS_ID, GELIC_DEV_ID, GELIC_CARD_TXINT|GELIC_CARD_RXINT, 0);

	result = unmap_dma_mem(GELIC_BUS_ID, GELIC_DEV_ID, gelic_var.dma_bus_addr, sizeof(dma));
	if (result != 0)
		return result;
