...
static void patch_self(char *src, char *dst, uint32_t sh_offset, uint32_t data_address, int extend)
{
	char *buf;
	FILE *f;
...
int main(int argc, char *argv[])
{
	...
	patch_self(self_input, "temp.self", sh_addr, data_addr, (patch_addr != 0xFFFFFFFF));
	
	command2("unself", "temp.self", "temp.elf");
	
	command3("self_rebuilder", "temp.elf", "temp2.self", "temp.self");
	...
	if (patch_addr != 0 && patch_addr != 0xffffffff)
	{
	...
	/*jump = SWAP32(MAKE_CALL_VALUE(payload_jump, payload_addr));	
	fseek(elf, text_offset+payload_jump, SEEK_SET);
	fwrite(&jump, 1, sizeof(jump), elf);*/
