...
#if defined(FIRMWARE_3_41) || defined(FIRMWARE_3_55)

#define AUTH_OFFSET		0x8B320
#define COMPAT_OFFSET		0x868D4
#define DISC_TYPE_OFFSET	0x89488

#define DATA_TOC_OFFSET		-0x5960
#define	LPAR_TOC_OFFSET		-0x5A20
#define FILESZ_TOC_OFFSET	-0x32E0
...
static INLINE uint8_t *get_buffer_from_lpar(uint64_t lpar)
{
	...
	DPRINTF("Warning: cannot find buffer for lpar: 0x%lx!!!\n", lpar);
...
{
	uint64_t *structure = (uint64_t *) ((0x468*fd) + *(uint64_t *)(TOC+FILESZ_TOC_OFFSET));
	return structure[0x170/8];
}
...
static INLINE void apply_debug_patches(void)
{
	...
	//hook_function_with_postcall(0x74AA4, muu, 1);
	//hook_function_with_precall(0x8603C, buu, 3);
	//hook_function_with_precall(cdvd_send_atapi_command_symbol, cdvd_send_atapi_command_hook, 3);	
}
...
static INLINE void apply_patches(void)
{
	...
	for (int i = 0; i < N_PATCHES; i++)
	{
		...
		memset(&patches[i], 0, sizeof(Patch));
	}	
}
...
#ifdef DEBUG
	debug_init();
	DPRINTF("Hello from ps2softemu_stage2.\n");	
#endif
