...
PS2EMU_HOOKED_FUNCTION_POSTCALL_3(void, reboot_parameters_hook, (uint64_t port, uint8_t *buf, uint64_t size))
{
	if (port == VUART_PORT_SYSMGGR && get_patched_func_call_address() == (void *)reboot_parameters_vuart_call)
	{
		memcpy(buf+0x4d9+0x18, (void *)arguments_symbol+0x4d9, 7);
		memcpy(buf+0x4e9+0x18, (void *)arguments_symbol+0x4e9, 7);
	}	
}
...
{
	hook_function_with_postcall(vuart_write_symbol, reboot_parameters_hook, 3);
