...
void TranslateCode(uint32_t PC, uint32_t size)
{
...
int main(int argc, char *argv[])
{
	...
	if (argc != 6)
	{
		printf("Usage: %s bin map h_out opcodes_out rom_keys\n", argv[0]);
		return -1;
	}
	...
	romsize = fread(ROM, 1, sizeof(ROM), bin);
	fclose(bin);
	...
	{		
		if(strstr(line, "CODE)") && strncmp(line, "CONST", 5) != 0)
		{
			...
			TranslateCode(addr, size);
		}
		else if (strstr(line, "_stage2"))
		{
			unsigned int unused;
	...
	if (stage2 == 0 || stage2_size == 0)
	{
		/*printf("Error: stage2 not found.\n");
		return -1;*/
	}
	
	FILE *k = fopen(argv[5], "rb");
	if (!k)
	{
		printf("Cannot open %s\n", argv[5]);
		return -1;
	...
	for (int i = 0; i < romsize; i++)
	{
		...
		{
			ROM[i] ^= rom_key[i % rom_keys_len];
		}
	}
		
	fprintf(h, "static char ROM[%d] __attribute__((aligned(16))) =\n{\n\t", romsize);
