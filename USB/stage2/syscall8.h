...
/* Compatibility layer for multiman */
#define SYSCALL8_OPCODE_HERMES_ENABLE			0x1
#define SYSCALL8_OPCODE_HERMES_MEMCPY			0x2
#define SYSCALL8_OPCODE_HERMES_PERM_MODE		0x8
#define SYSCALL8_OPCODE_HERMES_PATHTABLE		0x9

extern uint64_t hermes_memcpy_dst;
extern uint64_t hermes_memcpy_src;

static INLINE int sys_hermes_memcpy(uint64_t dst, uint64_t src, uint64_t size)
{
	DPRINTF("Hermes memcpy: %lx %lx %lx\n", dst, src, size);
	hermes_memcpy_dst = dst;
	hermes_memcpy_src = src;
	return 0;
}
