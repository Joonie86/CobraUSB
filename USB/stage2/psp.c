...
#ifdef FIRMWARE_3_55
#define prx_patch_call_lr	0x5A2F0
#define umd_mutex_offset	(0x52680+0x384)
#endif

// 4.00
#define emu2_prx_patch_call_lr	0x5831C
#define emu2_umd_mutex_offset	(0xF4B00+0x38C)
...
#ifdef DEBUG
static process_t pspemu_process;
#endif
...
#ifdef DEBUG
static __attribute__((unused)) void debug_thread(uint64_t arg)
{
	timer_usleep(MILISECONDS(500));
	
	while (1)
	{
		int ret;
		static uint8_t dbg_buf[512];
		
		DPRINTF("-------------------\n");
		ret = copy_from_process(pspemu_process, (void *)0x80000000, dbg_buf, sizeof(dbg_buf));
		if (ret == 0)
		{
			for (int i = 0; i < sizeof(dbg_buf); i++)
			{
				DPRINTF("%02X ", dbg_buf[i]);
				if ((i&0xF) == 0xF)
					DPRINTF("\n");
			}
		}
...
ENCRYPTED_FUNCTION(int, sys_psp_read_header, (int fd, char *buf, uint64_t nbytes, uint64_t *nread))
{
	int ret;
	uint32_t n, unk2;
	uint64_t umd_size;
...
#ifdef DEBUG
	/*thread_t thread;
	pspemu_process = process;
	ppu_thread_create(&thread, debug_thread, 0, -0x1D8, 0x1000, 0, "PSP debug thread");	*/
#endif
...
ENCRYPTED_FUNCTION(int, sys_psp_read_umd, (int unk, void *buf, uint64_t sector, uint64_t ofs, uint64_t size))
{
	...
	if (!mutex)
	{
		...
		if (ret != 0)
		{
			ret = open_kernel_object(object_table, *(uint32_t *)(emulator_api_base+emu2_umd_mutex_offset), (void **)&mutex, &obj_handle, SYS_MUTEX_OBJECT);
		}
...
ENCRYPTED_FUNCTION(int, sys_psp_set_umdfile, (char *file, char *id, int prometheus))
{
	int ret;
...
ENCRYPTED_FUNCTION(int, sys_psp_set_decrypt_options, (int decrypt_patch, uint32_t tag, uint8_t *keys, uint8_t code, uint32_t tag2, uint8_t *keys2, uint8_t code2))
{
	if (!umd_file)
		return EABORT;
...
ENCRYPTED_FUNCTION(int, sys_psp_prx_patch, (uint32_t *unk, uint8_t *elf_buf, void *link_register))
{
	...
	if (link_register == (void *)(pemucorelib_base+prx_patch_call_lr) || link_register == (void *)(pemucorelib_base+emu2_prx_patch_call_lr))
	{		
...
ENCRYPTED_FUNCTION(int, sys_psp_set_emu_path, (char *path))
{
	if (!path)
	{
...
ENCRYPTED_FUNCTION(int, sys_psp_post_savedata_initstart, (int result, void *param))
{
...
ENCRYPTED_FUNCTION(int, sys_psp_post_savedata_shutdownstart, (void))
{
